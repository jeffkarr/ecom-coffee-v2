...
const CoffeePage = () => {
  ...
  useEffect( () => {
    const fetchCoffeeItems = async () => {
      try {
        const res = await fetch('http://localhost:5000/coffeeItems');
  ...
  const onAddCoffeeToCart = (e) => {
    ...
    let tempCartItem = coffees.filter(coffeeItem => coffeeItem.itemId === cartCoffeeItemId);
  ...
  const onAddCoffeeToWish = (e) => {
    ...
    let tempWishItem = coffees.filter(coffeeItem => coffeeItem.itemId === wishCoffeeItemId);
